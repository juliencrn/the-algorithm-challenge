...
We’ve started writing a `PriorityQueue` in the code editor. You will need to add an `addOne()` method for adding items with a priority, a `removeOne()` method for removing and returning items, a size method to return the number of items in the queue, a `next()` method to return the element at the front of the queue, and finally an `isEmpty()` method that will return true if the queue is empty or false if it is not.

The `addOne()` should accept items with the format shown above `(['human', 1])` where 1 represents the priority. `next()` and `removeOne()` should return only the item's name, not its priority.
